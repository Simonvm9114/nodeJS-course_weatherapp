...
const forecast = (latitude, longitude, callback) => {
    ...
    request({ url, json: true }, (error, { body }) => {
        if (error) {
            ...
            const {weather_descriptions: describtion, temperature, precip} = body.current
            callback(undefined, `${describtion[0]}. It's currently ${temperature} degrees out. There is a ${precip}% chance of rain.`)
        }
    })
}
